...
The one prohibited action in the pilot came not from a wrong status (\sP{} was the gold status) but from a status that named no action. The agent filled the gap with the action that the lowest-standing source requested (O1). The HUMAN blocks avoided this because they listed permitted and prohibited actions. The same lesson follows from O2: an \sAMB{} result that still names a governing directive is read as a decision. We draw three design rules from these traces:
\begin{enumerate}
