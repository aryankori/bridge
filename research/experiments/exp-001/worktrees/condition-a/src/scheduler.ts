...
export class TaskScheduler {
 ...
 // BUG 2: Unclamped token refill allows tokens to grow beyond capacity
 private refillTokens(): void {
 const now = Date.now();
 const elapsedSeconds = (now - this.lastRefillTimestamp) / 1000;
 if (elapsedSeconds > 0) {
 const addedTokens = elapsedSeconds * this.refillRatePerSecond;
 this.tokens = Math.min(this.capacity, this.tokens + addedTokens); // BUG: Missing Math.min(this.capacity, ...)
 ...
 // BUG 3: Sorting solely by priority without sequenceNumber tie-breaker reverses or breaks FIFO stability
 this.queue.sort((a, b) => PRIORITY_SCORES[b.options.priority] - PRIORITY_SCORES[a.options.priority]);
 ...
 // BUG 1: Double decrement when task is aborted/cancelled during execution
 this.activeCount--;
 this.pump();
 }
 }
